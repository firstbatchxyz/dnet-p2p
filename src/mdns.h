 ...
 *
 * 2025-03: Modified by @erhant with main function & docs
 */
...
/// A wrapper for a string with known length.
...
/// A [SRV](https://www.cloudflare.com/learning/dns/dns-records/dns-srv-record/) record
...
/// PTR record, pointer to another location in the domain name space
...
/// Arbitrary TXT record.
struct mdns_record_txt_t {
  mdns_string_t key;
  mdns_string_t value;
};

/// An arbitrary DNS record.
...
/// A DNS query for specific rtype.
struct mdns_query_t {
  mdns_record_type_t type;
  const char* name;
  size_t length;
...
/**
 * Open and setup a IPv4 socket for mDNS/DNS-SD.
 * 
 * - To bind the socket to a specific interface, pass in the appropriate socket
 * address in `saddr`, otherwise pass a null pointer for `INADDR_ANY`.
 * 
 * - To send one-shot discovery requests and queries pass a null pointer or set
 * 0 as port to assign a random user level ephemeral port.
 * 
 * - To run discovery service listening for incoming discoveries and queries,
 * you must set MDNS_PORT as port.
 */
static inline int mdns_socket_open_ipv4(const struct sockaddr_in* saddr);

/**
 * Setup an already opened IPv4 socket for mDNS/DNS-SD.
 * 
 * - To bind the socket to a specific interface, pass in the appropriate socket
 * address in saddr, otherwise pass a null pointer for INADDR_ANY.
 * 
 ...
 * 0 as port to assign a random user level ephemeral port.
 * 
 * - To run discovery service listening for incoming
...
/**
 * Open and setup a IPv6 socket for mDNS/DNS-SD.
 * 
 * - To bind the socket to a specific interface, pass in the appropriate socket
 * address in `saddr`, otherwise pass a null pointer for `in6addr_any`.
 * 
 * - To send one-shot discovery requests and queries pass a null pointer or set
 * 0 as port to assign a random user level ephemeral port.
 * 
...
/**
 * Setup an already opened IPv6 socket for mDNS/DNS-SD.
 * 
 * - To bind the socket to a specific interface, pass in the appropriate socket
 * address in `saddr`, otherwise pass a null pointer for `in6addr_any`.
 * 
...
/**
 * Close a socket opened with `mdns_socket_open_ipv4` and
 * `mdns_socket_open_ipv6`.
 * 
 * Simply calls the `close` system call on the socket.
 * 
 * @TODO: doesnt this also work for sockets with setup?
 */
static inline void mdns_socket_close(int sock);

/**
 * Listen for incoming multicast DNS-SD and mDNS query requests.
 * 
 * The socket should have been opened on port MDNS_PORT using one of the mdns
 * `socket_open` or `socket_setup` functions.
 * 
 * Buffer must be 32 bit aligned.
 * Parsing is stopped when callback function returns non-zero.
 * 
 * @return number of queries parsed.
 */
...
/**
 * Recieve unicast responses to a DNS-SD sent with mdns_discovery_send.
 * 
 * Any data will be piped to the given callback for parsing.
 * 
 * Buffer must be 32 bit aligned.
 * Parsing is stopped when callback function returns non-zero.
 * 
 * @return the number of responses parsed.
 */
 ...
 *
 * The supplied buffer will be used to build the query packet and must be 32 bit aligned.
 *
 * The query ID can be set to non-zero to filter responses, however the RFC
 * states that the query ID SHOULD be set to 0 for multicast queries.
 *
 * The query will request:
 * - a unicast response if the socket is bound to an ephemeral port, or
 * - a multicast response if the socket is bound to mDNS port 5353.
 *
 * @return the used query ID, or `< 0` if error.
 */
 ...
 * bit aligned. 
 * 
 * The query ID can be set to non-zero to filter responses,
 * however the RFC states that the query ID SHOULD be set to 0 for multicast
 * queries. 
 * 
 * Each additional service name query consists of a triplet: 
 * 
 * - a record type `mdns_record_type_t`.
 * - a name string pointer `const char*`.
 * - a name length `size_t`.  
 * 
 * The list of variable arguments should be terminated with a record type of 0. 
 * 
 * The query will request a unicast response if the socket is
 ...
 * optionally filtering out any responses not matching the given query ID.
 * 
 * Set the query ID to 0 to parse all responses, even if it is not matching the
 ...
 * number of records to the given address.
 *
 * Use the top bit of the query class
 * field (MDNS_UNICAST_RESPONSE) in the query recieved to determine if the
 * answer should be sent unicast (bit set) or multicast (bit not set).
 *
 * Buffer must be 32 bit aligned. 
 * The record type and name should match the data from the query recieved.
 ...
 * number of records.
 *
 * Use the top bit of the query class field
 ...
 * 
 * Use this on service startup to announce your instance to the local network.
 * 
 * Buffer must be 32 bit aligned. 
 * 
 * @return `0` if success, or `< 0` if error. 
...
/**
 * Send a variable multicast mDNS announcement.
 * 
 * Use this on service end for removing the resource from the local network.
 * The records must be identical to the according "announcement".
 */
...
/** 
 * Parse a `SRV` record
 * @return the priority, weight, port and name in the record
 */
...
///
/// - If the strings are equal it returns >0 and the offset variables are
/// updated to the end of the corresponding strings.
/// - If the strings are not equal it returns 0 and the offset variables are NOT
/// updated.
...
static inline void mdns_socket_close(int sock) { close(sock); }
...
static inline int mdns_multicast_send(int sock, const void* buffer, size_t size) {
  ...
  // get the address of the currently specificed socket
...
#endif
    // use ipv6 address FF02::FB for multicast, as per the RFC
...
#endif
    // use ipv4 address 224.0.0.251 for multicast, as per the RFC
  ...
  // send UDP packet `*buffer` to the multicast address
...
static inline size_t mdns_discovery_recv(int sock, void* buffer, size_t capacity, mdns_record_callback_fn callback,
  ...
  // recieve UDP packet into `*buffer`
...
static inline void* mdns_answer_add_record(void* buffer, size_t capacity, void* data, mdns_record_t record,
  ...
  switch (record.type) {
    case MDNS_RECORDTYPE_PTR:
      data =
          mdns_string_make(buffer, capacity, data, record.data.ptr.name.str, record.data.ptr.name.length, string_table);
      break;
    ...
    case MDNS_RECORDTYPE_SRV:
      ...
      data =
          mdns_string_make(buffer, capacity, data, record.data.srv.name.str, record.data.srv.name.length, string_table);
      break;

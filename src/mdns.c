...
static int dump_callback(int sock, const struct sockaddr* from, size_t addrlen,
                         mdns_entry_type_t entry, uint16_t query_id,
                         uint16_t rtype, uint16_t rclass, uint32_t ttl,
                         const void* data, size_t size, size_t name_offset,
                         size_t name_length, size_t record_offset,
...
static int dump_mdns(void) {
  ...
  while (running) {
    ...
    if (select(nfds, &readfs, 0, 0, &timeout) >= 0) {
      for (int isock = 0; isock < num_sockets; ++isock) {
        if (FD_ISSET(sockets[isock], &readfs)) {
          mdns_socket_listen(sockets[isock], buffer, capacity, dump_callback,
                             0);
        }
        FD_SET(sockets[isock], &readfs);
      }
...
#define MAX_PASSES (1024 * 1024 * 1024)
  ...
  static uint8_t fuzz_mdns_services_query[] = {
      ...
      0x00, 0x09, '_',  's',  'e',  'r',  'v',  'i',  'c',  'e',  's',
      0x07, '_',  'd',  'n',  's',  '-',  's',  'd',  0x04, '_',  'u',
      'd',  'p',  0x05, 'l',  'o',  'c',  'a',  'l',  0x00};
  ...
  for (int ipass = 0; ipass < MAX_PASSES; ++ipass) {
    ...
    for (size_t i = 0; i < size; ++i) buffer[i] = rand() & 0xFF;

    if (ipass % 4) {
      // Crafted fuzzing, make sure header is reasonable
      memcpy(buffer, fuzz_mdns_services_query,
             sizeof(fuzz_mdns_services_query));
      uint16_t* header = (uint16_t*)buffer;
      header[0] = 0;
      header[1] = htons(0x8400);
      for (int ival = 2; ival < 6; ++ival) header[ival] = rand() & 0xFF;
    }
    ...
    mdns_record_parse_ptr(buffer, size, offset, length, strbuffer,
                          MAX_FUZZ_SIZE);

    offset = size ? (rand() % size) : 0;
    length = size ? (rand() % (size - offset)) : 0;
    mdns_record_parse_srv(buffer, size, offset, length, strbuffer,
                          MAX_FUZZ_SIZE);
    ...
    offset = size ? (rand() % size) : 0;
    length = size ? (rand() % (size - offset)) : 0;
    mdns_record_parse_txt(buffer, size, offset, length,
                          (mdns_record_txt_t*)strbuffer, MAX_FUZZ_SIZE);

    if (ipass && !(ipass % 10000)) printf("Completed fuzzing pass %d\n", ipass);
  }
...
int mdns_main(int argc, const char* const* argv) {
  ...
  for (int iarg = 0; iarg < argc; ++iarg) {
    if (strcmp(argv[iarg], "--discovery") == 0) {
      ...
      mode = 2;
      ++iarg;
      if (iarg < argc) service = argv[iarg];
    } else if (strcmp(argv[iarg], "--dump") == 0) {
      mode = 3;
    } else if (strcmp(argv[iarg], "--hostname") == 0) {
      ++iarg;
      if (iarg < argc) hostname = argv[iarg];
    } else if (strcmp(argv[iarg], "--port") == 0) {
      ++iarg;
      if (iarg < argc) service_port = atoi(argv[iarg]);
    }
  }
